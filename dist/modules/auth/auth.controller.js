...
router.post('/register', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`📝 Iniciando registro: ${email} - IP: ${(0, helpers_1.getClientIP)(req)}`);
    const { user, emailSent } = await auth_service_1.default.register({ name, email, password, referralCode });
    logger_1.logger.info(`✅ Registro concluído: ${email} - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.post('/login', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`🔐 Tentativa de login: ${email} - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.get('/verify-email', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`📧 Verificando email com token: ${token} - IP: ${(0, helpers_1.getClientIP)(req)}`);
    const { user } = await auth_service_1.default.verifyEmail(token);
    logger_1.logger.info(`✅ Email verificado: ${user.email} - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.post('/forgot-password', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { email } = req.body;
    logger_1.logger.info(`🔑 Recuperação de senha solicitada: ${email} - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.post('/reset-password', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`🔑 Reset de senha token: ${token} - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.post('/refresh-token', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`🔄 Renovando token - IP: ${(0, helpers_1.getClientIP)(req)}`);
...
router.post('/logout', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { refreshToken } = req.body;
    logger_1.logger.info(`👋 Logout solicitado - IP: ${(0, helpers_1.getClientIP)(req)}`);
